...
export async function GET(request: Request) {
  try {
    ...
    for (const c of SHARES_PRIORITY) {
      const all = getUnitEntries(usgaap, c);
      if (all.length > sharesAll.length) sharesAll = all;
    }
    const shares = norm(dedupeByEnd(sharesAll));
    ...
    return NextResponse.json({
      ...
      grossProfit: { annual: norm(grossProfit.annual), quarter: norm(grossProfit.quarter) },
